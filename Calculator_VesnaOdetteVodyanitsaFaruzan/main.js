...
const additionalAttributeParams = {
  ...
  const defaultCharacterArtifacts = Object.entries(characters).reduce((result, item) => { 
      result[item[0]] = structuredClone(item[1].artifacts);
      return result;
    }, {});
  let n_char = Object.keys(characters).length;
  ...
  function update_onfield_character(action){
    
    if(action.talentMeta.isOnfield){onfieldCharacterID = action.talentMeta.characterID};
  }
  ...
  const characterEffects = Object.keys(characters).reduce((result, obj) => {
  ...
  function merge_stat_buff_details(statBuffDetails, desc, effectBuff){
    ...
    for(let [k, v] of Object.entries(effectBuff)){
      if(STAT_KEY_SET.has(k)){ 
        ...
        statBuffDetails[k].push(effectName + "：" + get_stat_buff_detail_value_string(k, v));
      }
    }
  }
  function derive_total_buff_from_effects(characterID, effects, teamInitialAttributes, teamNetAttributes, action, 
    ...
    for(let effect of effects){
      if(!ineffectiveEffectIDSet.has(effect.ID) && !ineffectiveEffectOwnerIDSet.has((effect.ID).split("_")[0])){
        let mark = true;
  ...
  const toUpdateAttributes = Object.keys(characters).reduce((result, ID) => {result[ID]=true; return result;}, {}); 
  ...
  function get_action_array(characters){
    ...
    const reactionStellarSwirlAnemoTalentMeta = {characterID: "Vesna", element:"anemo", rxndmg:"reactionStellarSwirl", 
                                                 ID:"reactionStellarSwirlAnemo", name:"反应星扩散:风"};
    ...
    const beginingActions = [
      ...
      {talentMeta:Odette.talentMetas.e2_swirl, ineffectiveEffectIDSet:baseIneffective},
    ...
    const VesnaActionsC0 = [
      ...
      {talentMeta:reactionStellarSwirlAnemoTalentMeta, repetitionCount:15,},
    ...
    for(let i = 0; i<n_action; i++){
      ...
      if(action.talentMeta.ID === "swap"){
        ...
        if(action.talentMeta.isSnapshot === true){
          ...
          curr_result.characterID = characterID;
          curr_result.onfieldCharacterID = onfieldCharacterID;
  ...
  function build_stat_detail_lines(statBaseDetails, statBuffDetails, statID){
    ...
    for(let k of keys){
      for(let detail of ((statBuffDetails || {})[k] || [])){ 
        ...
        const valueText = detail.slice(idx + 1);
        lines.push({name: detail.slice(0, idx), valueText: valueText, cls: "buff-value", 
                    stat:k, value:convert_numberText_to_number(valueText)});
      }
    }
    return lines;
  }
  ...
  function display_damage_details(wrapDetail, result){
    ...
    if(dmgType === "reactionLunar" || dmgType === "reactionStellar"){
      ...
      const scaling = result.details[ID] ? (result.details[ID].scaling ? result.details[ID].scaling:null) : null;
      if(scaling){
        const stats = result.details[ID].stats;
        ...
        scalingRow.append(mkExprBox(result.details[ID].baseMult, MULTIPLIERS["baseMult"], "baseMult"), mkExprOp("="));
  ...
  create_character_build_part("character_build_part");
  create_display_part("character_display_part");
  create_damage_display_part("damage_display_part");
